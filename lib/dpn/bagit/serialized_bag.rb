...
class DPN::Bagit::SerializedBag
  ...
  def initialize(_location)
    raise ArgumentError, "File does not exist!" if File.exist?(_location) == false
  ...
  def unserialize!
    ...
    raise "cannot untar #{file}" unless $CHILD_STATUS.success?
    name = File.basename(@location).to_s.sub(/\..*/, '') # remove the file extension
    DPN::Bagit::Bag.new(File.join(File.dirname(@location), name))
  end
end
